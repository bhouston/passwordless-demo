...
export function PasskeyComponent({ userId, hasPasskey, userName, userDisplayName }: PasskeyComponentProps) {
  ...
  const deletePasskeyMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : 'Failed to delete passkey. Please try again.');
      setSuccessMessage(null);
    },
  });
  ...
  return (
    <div className="border border-border bg-card p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Manage your passkey for secure, passwordless authentication</p>
      </div>
      ...
      <FieldSet>
        <FieldGroup>
          ...
          <Field>
            {hasPasskey ? (
              <Button onClick={handleDeletePasskey} disabled={isLoading} variant="destructive">
