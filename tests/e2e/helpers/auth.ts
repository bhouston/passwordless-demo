...
async function getLatestOtpPayload(
	type: OtpType,
	email: string,
...
export async function signUpViaOtp(
	...
	const { code: signupCode, token } = await getLatestOtpPayload(
		"signup-otp",
		email,
	);
	...
	await page
		.getByText(/Enter the 8-character/)
...
export async function loginViaEmailCode(
	...
	const { code: loginCode, token } = await getLatestOtpPayload(
		"login-otp",
		email,
	);
	...
	await expect(page).toHaveURL(
		(url) => url.pathname === `/login-via-code/${token}`,
		{ timeout: 15000 },
	);
