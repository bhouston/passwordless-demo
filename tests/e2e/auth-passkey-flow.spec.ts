...
test.describe('Auth passkey flow', () => {
  ...
  test('signup -> register passkey -> logout -> login with passkey', async ({ page }) => {
    ...
    await expect(page.getByText(/Passkey registered successfully!/i)).toBeVisible();
  ...
  test('shows registration cancelled when passkey setup is cancelled', async ({ page }) => {
  ...
  test('shows an authentication cancelled error', async ({ page }) => {
    ...
    await expect(page.getByText(/Authentication was cancelled or not allowed by your device\./i)).toBeVisible();
  });

  test('shows a missing passkey error when no credential is available', async ({ page }) => {
    await setNextAuthenticationError(page, {
    ...
    await expect(page.getByText(/No passkey found\. Please login with an email code instead\./i)).toBeVisible();
  });

  test('shows the unsupported browser message when WebAuthn is unavailable', async ({ page }) => {
